...
* `--igvm-personality <pcat|uefi|linux-direct>`: Select the chipset and
  ...
  The `pcat` and `uefi` personalities use the corresponding Gen1 or Gen2
  device shape, but firmware is loaded from the IGVM. They do not select the
  normal external-PCAT or external-UEFI load path. The `linux-direct`
  personality enables Hyper-V enlightenments only when `--hv` is also
  specified. PCAT and UEFI personalities require Hyper-V enlightenments, and
  fail explicitly on backend and isolation combinations that cannot provide
  them.
